...
| File | Responsibility |
|---|---|
| `firmware/src/core/settings/SettingsValidation.h` (create) | Pure parse/normalize/validate + `urlEncode` + `rgb565` conversions + JPEG header check. std only. |
...
| `firmware/src/core/ota/OtaUpdater.{h,cpp}` (modify) | Expose `server()` and `authorized()`. |
...
- `bool normalizeTickers(const std::string &in, std::string &out, std::string &err)` — split on `,`, trim, drop empties, 1–5 items, chars `[A-Za-z0-9./&=:_^-]`, upper-case up to the first `&`, rejoin with `,`, total ≤ 128.
- `bool normalizeText(const std::string &in, size_t maxLen, std::string &out, std::string &err)` — trim, collapse runs of spaces, 1–maxLen, reject control chars.
- `std::string urlEncode(const std::string &in)` — RFC 3986 unreserved kept, rest `%XX` upper-case.
- `bool parseHexColor(const std::string &in, uint16_t &rgb565, std::string &err)` — `#rrggbb`.
...
- [ ] **Step 3:** Write the tests (all cases below), with a header-only stub so the file compiles and the tests FAIL:
  ...
  - color: `"#fc8000"`→ `0xFC00`; `"fc8000"` err; `"#zzzzzz"` err; `rgb565ToHex(0xFFFF)`→`"#ffffff"`, `rgb565ToHex(0)`→`"#000000"`.
...
- `ClockWidget`: `CLOCK_COLOR`/`CLOCK_SHADOW_COLOR` uses → `m_color`/`m_shadow` members set in ctor from settings; `FORMAT_24_HOUR`→`Settings::get().h24`; `SHOW_AM_PM_INDICATOR`→`.ampm`; `m_type` from `.face`, clamped to NORMAL if the face's images are not compiled in.
...
- [ ] **Step 3:** OTA-flash; the orb must look exactly as before (clock face, colors, rotation 15 s, tickers, weather). Check the weather orb shows real data; record whether the URL-encoding changed anything.
- [ ] **Step 4:** Commit.
...
**Interfaces — Consumes:** `Settings`, `sv::*`. **Produces:** `class SettingsPage { public: explicit SettingsPage(OtaUpdater &ota); void begin(); };` (idempotent, call after `otaUpdater->begin()`). `OtaUpdater` gains `WebServer &server();` and `bool authorized();` (returns false after sending the 401 challenge).

- [ ] **Step 1:** OtaUpdater accessors; `/update` handlers use `authorized()`.
- [ ] **Step 2:** `GET /settings`: form with every v1 field, current value, config.h default as hint, color pickers, hour selects, face select (Nixie/Custom only if compiled in), running commit + uptime, links to `/update`. All values through `htmlEscape`. Boot-picture section placeholder for Task 4.
...
- [ ] **Step 4:** Page JS: file input → canvas 240×240 center-crop → circle preview → `toBlob('image/jpeg', q)` stepping q from 0.85 by 0.1 until ≤ 64 KB → `fetch` POST FormData → show server reply.
